...
<body>
                        ...
                        <div class="mt-auto">
                            ...
                            <form action="{{ route('editor.finish', $job->id) }}" method="POST" class="finish-form" data-job="{{ $job->id }}">
                                ...
                                <div class="d-flex gap-2">
                                    <a href="{{ route('jobs.show', $job->id) }}" class="btn btn-outline-secondary btn-sm flex-grow-1 fw-bold" target="__blank">Detail</a>
                            ...
                            <form action="{{ route('editor.start', $job->id) }}" method="POST">
                                @csrf
                                <div class="d-flex gap-2">
                                    <a href="{{ route('jobs.show', $job->id) }}" class="btn btn-outline-primary w-100 btn-sm fw-bold" target="__blank">Detail</a>
            ...
            <div class="modal-content border-0 rounded-4 shadow">
                ...
                <div class="modal-body">
                    ...
                    <div class="alert alert-light small text-muted rounded-3">
                        Pastikan link sudah benar sebelum menyelesaikan job.
                    </div>
                </div>
    ...
    @if(auth()->user()->email === 'crewizzati@web.com' && !session()->has('crew_verified'))
            ...
            <div class="modal-content border-0 rounded-4 shadow">
                ...
                <div class="modal-header border-0">
                    <h5 class="modal-title fw-bold">
                        <i class="bi bi-shield-lock me-2"></i> Verifikasi Crew
                    </h5>
                </div>

                <form method="POST" action="{{ route('editor.verify-crew') }}">
                    @csrf
                    <div class="modal-body">
                        <p class="text-muted small">
                            Masukkan email & password crew untuk melihat job.
                        </p>

                        <input type="email" name="email"
                            class="form-control mb-3"
                            placeholder="Email Crew" required>

                        <input type="password" name="password"
                            class="form-control"
                            placeholder="Password Crew" required>
                    ...
                    <div class="modal-footer border-0 d-flex flex-column gap-2">
                        ...
                        <!-- SUBMIT VERIFIKASI (KENA VALIDASI) -->
                        <button type="submit" class="btn btn-dark w-100 fw-bold">
                            Verifikasi & Masuk
                        </button>

                        <!-- CANCEL (LEWATKAN VALIDASI HTML) -->
