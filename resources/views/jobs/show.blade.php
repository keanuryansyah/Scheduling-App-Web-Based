    ...
    <div class="container py-5">
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-4">
                ...
                <!-- CARD KEUANGAN -->
                <div class="card border-0 shadow-lg overflow-hidden mb-4" style="border-radius: 24px;">
                    <!-- Bagian Atas: Header Info -->
                    <div class="card-body p-4 text-center" style="background: linear-gradient(135deg, #6366f1 0%, #a855f7 100%);">
                        <p class="text-white-50 small fw-bold text-uppercase mb-2" style="letter-spacing: 2px;">Total Nilai Job</p>
                        <div class="d-flex justify-content-center align-items-center mb-3">
                            <span class="text-white fw-bold me-2" style="font-size: 1.5rem; opacity: 0.8;">Rp</span>
                            <span class="text-white fw-bold display-4" style="line-height: 1;">{{ number_format($job->amount, 0, ',', '.') }}</span>
                        </div>

                        <!-- Badge Status -->
                        @php $isUnpaid = ($job->payment_method == 'unpaid' || $job->amount <= 0); @endphp
                            <div class="d-inline-flex align-items-center px-4 py-2 rounded-pill bg-white shadow-sm">
                            <i class="bi {{ $isUnpaid ? 'bi-exclamation-circle-fill text-danger' : 'bi-check-circle-fill text-success' }} me-2"></i>
                            <span class="fw-bolder small text-uppercase {{ $isUnpaid ? 'text-danger' : 'text-success' }}" style="letter-spacing: 0.5px;">
                                {{ $isUnpaid ? 'Belum Lunas' : 'Lunas' }}
                                @if($job->payment_method != 'unpaid') ({{ strtoupper($job->payment_method) }}) @endif
                            </span>
                    </div>
                </div>

                <!-- Bagian Bawah: Form (Tombol di bawah) -->
                @if(in_array(auth()->user()->role_id, [1, 2]))
                @if($job->status != 'scheduled')
                <div class="card-footer bg-white border-0 p-4">
                    <form action="{{ route('jobs.quick-update', $job->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row g-3">
                            <!-- Input Nominal -->
                            <div class="col-6">
                                <label class="form-label fw-bold text-secondary small mb-1">UPDATE NOMINAL</label>
                                <div class="input-group border rounded-3 shadow-sm overflow-hidden">
                                    <span class="input-group-text bg-light border-0 text-muted fw-bold">Rp</span>
                                    <input type="number" name="amount" class="form-control border-0 fw-bold"
                                        value="{{ (int)$job->amount > 0 ? (int)$job->amount : '' }}" placeholder="0">
                                </div>
                            </div>

                            <!-- Select Metode -->
                            <div class="col-6">
                                <label class="form-label fw-bold text-secondary small mb-1">METODE</label>
                                <select name="payment_method" class="form-select border shadow-sm rounded-3 fw-semibold">
                                    <option value="unpaid" {{ $job->payment_method == 'unpaid' ? 'selected' : '' }}>❌ Belum Lunas</option>
                            ...
                            <!-- Tombol Update Full-Width di Bawah -->
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg rounded-4">
                ...
                <div class="modal-body text-center p-4">
                    ...
                    <h4 class="fw-bold mb-2">Verifikasi Pembayaran</h4>
                    <!-- <p class="text-muted mb-4">
                        Setelah dikonfirmasi, pembayaran akan
                        <strong>masuk ke kas Boss</strong> dan
                        <strong>tidak bisa dibatalkan</strong>.
                    </p> -->

                    {{-- FORM FINAL --}}
                    <form action="{{ route('jobs.quick-update', $job->id) }}"
                        method="POST"
                        ...
                        <input type="hidden" name="payment_method" id="confirm_payment_method">
    ...
    <script>
        document.getElementById('confirmPaymentModal')
            .addEventListener('show.bs.modal', function() {
                ...
                document.getElementById('confirm_payment_method').value = method;
            });
