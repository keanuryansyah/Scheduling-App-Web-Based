...
class JobController extends Controller
{
    ...
    // METHOD BARU UPDATE AMOUNT
    public function quickUpdate(Request $request, Job $job)
    {
        // Proteksi role
        ...
        $request->validate([
            ...
            'payment_method' => 'required|in:unpaid,cash,tf,vendor',
        ]);
        ...
        if ($boss) {
            // 1. Cari apakah transaksi untuk job ini sudah pernah dicatat sebelumnya
            $existingTransaction = \App\Models\Transaction::where('job_id', $job->id)
                ->where('type', 'income')
                ->first();

            if ($existingTransaction) {
                // JIKA SUDAH ADA:
                // Selisihkan saldo (Kurangi saldo lama, tambah saldo baru) agar balance tetap akurat
                $boss->decrement('income', $existingTransaction->amount);
                $boss->increment('income', $job->amount);

                // Update record transaksi
                $existingTransaction->update([
                    'amount' => $job->amount,
                    'description' => 'Update Pemasukan Job: ' . $job->job_title . ' (' . strtoupper($request->payment_method) . ')',
                    'transaction_date' => now()
                ]);
            } else {
                // JIKA BELUM ADA:
                // Increment saldo Boss secara penuh
                $boss->increment('income', $job->amount);
        ...
        return back()->with('success', 'Data pembayaran job berhasil diperbarui.');
    }
