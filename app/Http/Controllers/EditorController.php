...
class EditorController extends Controller
{
    ...
    {
        $isGlobalEditor = auth()->user()->email === 'crewizzati@web.com';
        $hasCrewLogin   = session()->has('crew_verified');

        // Kalau global editor tapi belum login kedua
        if ($isGlobalEditor && !$hasCrewLogin) {
        ...
        // ID yang dipakai untuk filter
        $editorId = $isGlobalEditor
            ? session('crew_verified.id') // crew login kedua
            : auth()->id();                // editor biasa

        // 1. JOB AKTIF (Idle & Editing)
        $activeJobs = Job::with(['type', 'assignments'])
            ->whereIn('editor_status', ['idle', 'editing'])
            ->when($isGlobalEditor, function ($q) use ($editorId) {
                // 🔒 GLOBAL EDITOR + LOGIN KEDUA
                // HANYA job milik crew tersebut
                $q->whereHas('assignments', function ($a) use ($editorId) {
                    $a->where('user_id', $editorId);
                });
            }, function ($q) use ($editorId) {
                // 👤 EDITOR BIASA
                $q->where(function ($sub) use ($editorId) {
                    $sub->where('status', 'done')
                        ->orWhereHas('assignments', function ($a) use ($editorId) {
                            $a->where('user_id', $editorId);
                        });
                });
            })
    ...
    public function verifyCrew(Request $request)
    {
        $request->validate([
        ...
        $crew = User::where('email', $request->email)
            ->whereHas('role', fn($q) => $q->where('name', 'crew'))
            ->first();

        if (!$crew || !Hash::check($request->password, $crew->password)) {
            return back()->withErrors([
                'crew_login' => 'Email atau password crew salah.'
            ]);
        }
